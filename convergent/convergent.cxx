...
#include "gi_topological_convergent_gradient_builder_grad_align.h"
...
typedef IndexCompareLessThan<RegularGridTrilinearFunction> Comparer;
...
int main(int argc, char** argv) {
	...
	int use_simple_steepest = 0;
	if (argc >= 9)
		sscanf(argv[8], "%d", &use_simple_steepest);
	...
	// -- start timing IO
	m_grid = new RegularGrid3D(Vec3i(X, Y, Z), Vec3b(per_x, per_y, per_z));
	...
	sprintf(timingname, "%s.conv.gtime.txt", argv[4]);
	...
	if (true) {
		MaxVLabelingType* maxVLabeling = new MaxVLabelingType(m_tgrid, m_func2);
		maxVLabeling->ComputeOutput();


		printf("topo function...\n");
		m_topofunc = new TopoFuncType();
		m_topofunc->setMeshAndFuncAndMaxVLabeling(m_tgrid, m_func2, maxVLabeling);
		printf("done\n");
	}
	else {
		//m_topofunc = new TopologicalExplicitDenseMeshFunction<GridType, float>();
	...
	if (use_simple_steepest == 1) {
		TopologicalRegionGrowingSimpleGradientBuilder<GridType, TopoFuncType, GradType> * tconvergent
			= new TopologicalRegionGrowingSimpleGradientBuilder<GridType, TopoFuncType, GradType>(m_topofunc, m_tgrid, labeling);
		printf("computing grad with simple thing\n");
		tconvergent->computeGradient();
		printf("done\n");
	}
	else {
		TopologicalConverventGradientBuilderGradAlign<GridType, TopoFuncType, GradType> * tconvergent
			= new TopologicalConverventGradientBuilderGradAlign<GridType, TopoFuncType, GradType>(m_topofunc, m_func2, m_tgrid, labeling);
		printf("computing grad\n");
		tconvergent->computeGradient();
		printf("done\n");
		for (int i = 1; i < tconvergent->m_timing_vector.size(); i++) {
			fprintf(gtiming, "Round%d %d %d %d\n", i - 1,
				std::chrono::duration_cast<std::chrono::milliseconds>(tconvergent->m_timing_vector[i - 1] - g_start_time).count(),
				std::chrono::duration_cast<std::chrono::milliseconds>(tconvergent->m_timing_vector[i] - g_start_time).count(),
				std::chrono::duration_cast<std::chrono::milliseconds>(tconvergent->m_timing_vector[i] - tconvergent->m_timing_vector[i - 1]).count());
		}
	}
	//for (INDEX_TYPE i = 0; i < m_tgrid->numCells(); i++) {
	//	if (! labeling->getAssigned(i)) {
	//		labeling->setMark(i, 0);
	//		labeling->setCritical(i, false);
	//		labeling->setDimAscMan(i, 0);
	//		labeling->setPair(i, 0);
	//	}
	//}
	//labeling->outputToFile("fartpoop.raw");

	//labeling->outputToFile(argv[4]);
...
////typedef MorseSmaleComplexBasic<FLOATTYPE, TopologicalRegularGrid3D, TopologicalExplicitDenseMeshFunction, DiscreteGradientLabeling> MSCType;
////typedef NumericIntegratorExpandingRegionStop<AdaptiveEulerAdvector3D<-1>, Comparer> IntegratorType;
////typedef NumericIntegratorRegionStop<AdaptiveEulerAdvector3D<-1>, Comparer> IntegratorType2;
////typedef NumericStreamlineIntegrator<AdaptiveEulerAdvector3D<-1> > StreamlineIntegratorType;
////typedef IsolatedRegionRemover<Comparer> RegionRemoverType;
////typedef NumericStreamlineIntegrator<AdaptiveEulerAdvector3D<-1> > StreamlineIntegratorType;
//
...
////#else
////    TopologicalRegularGrid3D *m_tgrid = new TopologicalRegularGrid3D(m_grid);
////#endif
////
...
////    //i->BeginIntegration();
////    RegularGrid3D* m_grid = new RegularGrid3D(Vec3i(X, Y, Z), Vec3b(per_x, per_y, per_z));
...
////	printf("done with numeric integration!\ncreating vertex to boundary labeling...\n");
////	TopologicalRegularGrid3D* topo_grid = new TopologicalRegularGrid3D(i2->GetGrid());
////	
...
//////		TopologicalRegularGrid3D::AllCellsIterator all_cell_iter(topo_grid, topo_index_partition[thread_num], topo_index_partition[thread_num + 1]);
...
//////		}
//////	}
//////	TopologicalRegularGrid3D::AllCellsIterator all_cell_iter(topo_grid);
//////	for (all_cell_iter.begin(); all_cell_iter.valid(); all_cell_iter.advance()) {
//////		INDEX_TYPE cell_id = all_cell_iter.value();
...
////	//testgrad->SetAll(0);
////	//TopologicalRegularGrid3D::AllCellsIterator it(topo_grid);
