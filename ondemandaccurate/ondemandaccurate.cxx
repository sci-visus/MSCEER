...
bool GetOptions(int argc, char** argv) {
	...
	if (argc >= 15)
		sscanf(argv[14], "%d", &output_debug_blocks);
...
int main(int argc, char** argv) {
	...
	NewRobinsType* first_robins = new NewRobinsType(g_grid, g_rgt_func, g_topo_grid, g_maxv_labeling, base_grad);
	first_robins->ComputePairing_sliding();
	...
	if (need_ASC_3 || need_DSC_3 || needsad) {
		...
		if (need_ASC_3 || needsad) {
         ...
         std::unordered_map<INDEX_TYPE, INT_TYPE> extmapASC;
			...
			if (output_debug_blocks) test_simp_reg_builder_asc->GetOutputLabels()->OutputToIntFile("extremal_asc.raw");
			...
			if (output_debug_blocks) newintegrator_asc->GetOutputLabels()->OutputToIntFile("newintegrator_asc.raw");
			...
			if (output_debug_blocks) newintegrator_asc->GetOutputLabels()->OutputToIntFile("newintegrator_cleaned_asc.raw");
	...
	if (!(need_ASC_1 || need_DSC_1)) {
		...
		std::vector<INDEX_TYPE> topo_index_partition;
#if 1
		if (output_debug_blocks) {
			char name[2048];
			sprintf(name, "boundaries_%dx%dx%d.raw", (int)g_topo_grid->numCellsAxis(0), (int)g_topo_grid->numCellsAxis(1), (int)g_topo_grid->numCellsAxis(2));
			restriction_labels->OutputToFile(name);
		}
#endif
		int num_threads;
#pragma omp parallel
