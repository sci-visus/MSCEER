/* gi_adaptive_euler_advector_2d.h
*
...
*/
#ifndef ADAPTIVE_EULER_ADVECTOR_2D_H
#define ADAPTIVE_EULER_ADVECTOR_2D_H

#include "base/gi_basic_types.h"
#include "base/gi_vectors.h"
#include "base/gi_regular_grid_2d.h"
#include "base/gi_regular_grid_bilinear_function.h"
...
namespace GInt {
	...
	template< class GridFuncType, int StepMultiplier>
	class AdaptiveEulerAdvector2D {
	protected:
...
#undef min
	...
	public:
		...
		AdaptiveEulerAdvector2D(RegularGrid2D* grid, GridFuncType* func, FLOATTYPE grad_thresh,
		...
		ADVECTION_EVENT AdvectThroughVoxelNoCheck(Vec2d& current_point, int& iterations_left);


		// starts at a point, identifies which grid voxel it is in, and advects the point until it either exits the voxel,
		// or hits an early termination condition. sets the output point to the new location, or the last location
		// before early termination. also sets the reason for return - either exiting voxel or early termination
		ADVECTION_EVENT AdvectThroughVoxelNearBoundary(Vec2d& current_point, int& iterations_left);
	...
	template< class GridFuncType, int StepMultiplier>
	AdaptiveEulerAdvector2D<GridFuncType, StepMultiplier>::AdaptiveEulerAdvector2D(RegularGrid2D* grid, GridFuncType* func, FLOATTYPE grad_thresh,
		FLOATTYPE error_thresh, AdvectionChecker2D* inside_voxel_advection_checker) :
		m_grid(grid), m_func(func), m_gradient_threshold(grad_thresh),
		m_error_threshold(error_thresh), m_inside_voxel_advection_checker(inside_voxel_advection_checker){
		//m_num_boundary = 0;
		//m_num_noboundary = 0;
	}
	...
	// before early termination. also sets the reason for return - either exiting voxel or early termination
	template< class GridFuncType, int StepMultiplier>
	ADVECTION_EVENT AdaptiveEulerAdvector2D<GridFuncType, StepMultiplier>::AdvectThroughVoxelNoCheck(Vec2d& current_point, int& iterations_left) {
		//m_num_boundary++;
		Vec2d surrounding_grad[4]; // used to cache surrounding gradient
		...
		Vec2l old_point_int_base = current_point; // figure out what the closest vertex is we are in
		m_func->GetGradSurroundingNoBoundaryCheck(old_point_int_base, surrounding_grad); // fill in local gradients
		Vec2d gradient = m_func->BiLinInterpGrad(current_point, old_point_int_base, surrounding_grad); // cache old gradient for old_point - used when we change stepsize but not location

		Vec2l half_step_point_1_int_base(-1, -1); // used to figure out if cached half-step gradients can be reused
		Vec2d old_half_step_point_1_surrounding[4]; // cache surrounding gradeints for half-step
		...
		while (iterations_left-- > 0) {
			...
			if (!(old_point == current_point)) {
				...
				//Vec2l new_base = current_point.IntFloor();
				//if (!(new_base == old_point_int_base)) {
				//	// we have exited the current voxel
				//	return OUT_OF_VOXEL;
				//}

				gradient = m_func->BiLinInterpGrad(current_point, old_point_int_base, surrounding_grad);
			...
			Vec2d euler_displacement = gradient * (StepMultiplier * stepsize);
			...
			//// incorrect method: just re-use gradient and trilininterp
			{
				...
				Vec2d half_step_displacement = (gradient * (stepsize * (StepMultiplier * 0.5)));
				...
				Vec2d half_step_gradient = m_func->BiLinInterpGrad(half_step_point_1, old_point_int_base, surrounding_grad);
			...
			//	Vec2l new_base = half_step_point_1.IntFloor();
			//	Vec2d half_step_gradient;
			//	if (!(new_base == old_point_int_base)) {
	...
	// before early termination. also sets the reason for return - either exiting voxel or early termination
	template< class GridFuncType, int StepMultiplier>
	ADVECTION_EVENT AdaptiveEulerAdvector2D<GridFuncType, StepMultiplier>::AdvectThroughVoxelNearBoundary(Vec2d& current_point, int& iterations_left) {
		//m_num_noboundary++;
		Vec2d surrounding_grad[4]; // used to cache surrounding gradient
		Vec2d old_point = current_point; // set the last poitn to current one since we initialize cache
		Vec2d half_step_point_1; // these points are used to check the error of the current stepsize

		// declare variables to store gradient, and initialize with gradient at start position
		Vec2l old_point_int_base = m_grid->Inbounds(current_point); // figure out what the closest vertex is we are in
