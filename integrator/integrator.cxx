...
typedef MorseSmaleComplexBasic<FLOATTYPE, TopologicalRegularGrid, TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>, GradType> MSCType;
...
int main(int argc, char** argv) {
	...
	TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>* m_topofunc;
	GradType *labeling;
	RobinsLabelingAlgorithm<GridType, TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>>  *robin;
	TopologicalGradientUsingAlgorithms<GridType, TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>, GradType>* topo_algs;
	MSCType* msc;
	...
	m_topofunc = new TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>();
	...
	robin = new RobinsLabelingAlgorithm<GridType, TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>>(m_topofunc, m_tgrid, labeling);
	...
	printf("about to set dim asc man\n");
	topo_algs = new TopologicalGradientUsingAlgorithms<GridType, TopologicalExplicitDenseMeshFunction<TopologicalRegularGrid, float>, GradType>(m_topofunc, m_tgrid, labeling);
	topo_algs->setAscendingManifoldDimensions();
	...
	for (auto it = s_r1saddles->output.begin();
		...
		MSCType::SurroundingLivingArcsIterator sit(msc);
