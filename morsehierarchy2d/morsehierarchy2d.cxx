...
typedef MorseSmaleComplexBasic<FLOATTYPE, TopologicalRegularGrid, FuncType, GradType> MSCType;
int main(int argc, char** argv) {
	...
	GridType *m_tgrid;
	TopologicalExplicitDenseMeshFunction<GridType, float>* m_topofunc;
	GradType *labeling;
	...
	TopologicalConverventGradientBuilder<GridType, FuncType, GradType>* tconvergent
		= new TopologicalConverventGradientBuilder<GridType, FuncType, GradType>(m_topofunc, m_tgrid, labeling);
	...
	msc->ComputeFromGrad();
	msc->ComputeHierarchy(999999999999.0); // hack - want max vs. min
	...
	DFO_MSCNodeIndexSelect<MSCType>* saddles = new DFO_MSCNodeIndexSelect<MSCType>();
	saddles->AddInputDFO(nodes);
	saddles->SetMscDFO(hierarchy);
	saddles->SetPassingIndices(false, true, false, false);
