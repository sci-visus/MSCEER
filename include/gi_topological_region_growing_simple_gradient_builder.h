...
namespace GInt {
	...
	protected:
		...
		virtual void initAssigned() {
			...
			typename MeshType::AllCellsIterator allit(mMesh);
		...
		virtual void initNumberUnpairedFacets() {
			...
			for (allit.begin(); allit.valid(); allit.advance()) {
				INDEX_TYPE cellid = allit.value();
				INT_TYPE temp_num = 0;
				typename MeshType::FacetsIterator facets(mMesh);
		...
		virtual bool lessThanAllUnassignedCofacets(const INDEX_TYPE& cellid) {
			...
			typename MeshType::CofacetsIterator cofacets(mMesh);
			typename MeshType::FacetsIterator facets(mMesh);
		...
		virtual void seedQueueWithDMinima(const DIM_TYPE& dim) {
			int counter = 0;
			typename MeshType::DCellsIterator d_cells(mMesh, dim);
		...
		virtual int pickFromCandidates(const INDEX_TYPE& cellid,
			...
			int minloc = 0;
			typename FuncType::DType minval = lowestFacetValue(candidates[minloc]);
			//printf("%d=%.2f\n", candidates[0], minval);
			for (int i = 1; i < candidates.size(); i++) {

				typename FuncType::DType otherval = lowestFacetValue(candidates[i]);
	...
	public:
		...
		virtual float lowestFacetValue(const INDEX_TYPE& cellid) {
			...
			for (facets.begin(cellid); facets.valid(); facets.advance()) {
				...
				float nvalue = lowestFacetValue(temp_neg);
				if (!init) {
