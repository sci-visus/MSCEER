...
#define TOPOLOGICAL_2D_RESTRICTED_EXPANDING_REGIONS2_H

#ifndef TOPOLOGICAL_2D_RESTRICTED_EXPANDING_REGIONS2_H
#define TOPOLOGICAL_2D_RESTRICTED_EXPANDING_REGIONS2_H
...
namespace GInt {
	...
	class Topological2DRestrictedExpandingRegions2 {
	...
	public:
		...
		TopologicalRegularGrid3D* m_topo_grid;
		...
		bool IsHighestPrimalEdgeInUNASSIGNEDRestrictedNeighborhood(INDEX_TYPE primal_edge_topo_id) const {
			...
			TopologicalRegularGrid3D::CofacetsIterator t_adjacent_primal_quad_iterator(m_topo_grid);
			TopologicalRegularGrid3D::FacetsIterator t_neighboring_primal_edge_iterator(m_topo_grid);
		...
		// returns true if all the higher neighbors are "certain"
		void Inspect_Higher_Certains2(bool& has_higher_nonlocal, bool& has_higher_uncertain, INDEX_TYPE primal_edge_topo_id,
			FLOATTYPE original_edge_value, std::set<INDEX_TYPE>& narrow_band, std::set<INDEX_TYPE>& processed_band) const {
			INDEX_TYPE tneg;
			has_higher_nonlocal = false;
			has_higher_uncertain = false;
			...
			for (t_adjacent_primal_quad_iterator.begin(primal_edge_topo_id);
				...
				for (t_neighboring_primal_edge_iterator.begin(t_adjacent_primal_quad_topo_id);
					...
					// skip any non-permissable edges
					if (m_restriction_labels->GetLabel(t_neighboring_primal_edge_topo_id) != 1
						/*|| m_prior_info->GetLabel(t_neighboring_primal_edge_topo_id) != -1*/) // inserted --
						continue;
					...
					if (IsGreater(t_neighboring_primal_edge_topo_id, primal_edge_topo_id)) {
						if (processed_band.count(t_neighboring_primal_edge_topo_id) == 0) {
							// then its not certain so we can't be certain
							has_higher_uncertain = true;

						}
						if (narrow_band.count(t_neighboring_primal_edge_topo_id) == 0) {
							has_higher_nonlocal = true;
						}
					}
					//if (has_higher_uncertain && has_higher_nonlocal)
				}
			}
			return ;
		}
		...
		virtual void MyProcess(INDEX_TYPE cellid) {}
		...
		RegionState* Expand_MergeRegions(RegionState* region)  {
			...
			while (!growing_front.empty()) {
				...
				bool has_higher_uncertain;
				bool has_higher_nonlocal;
				Inspect_Higher_Certains2(has_higher_nonlocal, has_higher_uncertain, currid, currid_value, local_region->narrow_band, local_region->processed_band);
				if (!(has_higher_nonlocal || has_higher_uncertain)) {
					// then we can process this cell
					MyProcess(currid);
	...
	public:
		...
		Topological2DRestrictedExpandingRegions2(
			TopologicalExplicitDenseMeshFunction* func,
			TopologicalRegularGrid3D* topo_grid,
		...
		DenseLabeling<int>* m_prior_info; // stores all "seen" stuff that is on the boundary
		...
		void BeginIntegration() {
			...
			m_prior_info = new DenseLabeling<int>(m_topo_grid->numCells());
			m_prior_info->SetAll(0);
...
#pragma omp single
				...
				TopologicalRegularGrid3D::DCellsIterator primal_edge_iterator(m_topo_grid, 1, topo_index_partition[thread_num], topo_index_partition[thread_num + 1]);
