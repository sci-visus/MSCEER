...
namespace GInt {
	...
	protected:
		...
		inline FLOATTYPE PrimalEdgeValue(INDEX_TYPE primal_edge_topo_id) const {
			...
			TopologicalRegularGrid3D::FacetsIterator t_adjacent_primal_vertex_iterator(m_topo_grid);
		...
		inline bool GetDualQuadFromPrimalEdgeWithGrad(INDEX_TYPE primal_edge_topo_id, int& proj_axis,
			...
			//TopologicalRegularGrid3D::CofacetsIterator primal_quads_iter(m_topo_grid);
		...
		void RecursiveIntegrateUpwardsFromPoint(INDEX_TYPE current_primal_edge_topo_id, Vec3d current_point_grid_coords,
			...
			int countsplits = 0;
			TopologicalRegularGrid3D::FacetsIterator next_edge_iter(m_topo_grid);
...
#pragma omp parallel
			{
				int thread_num = omp_get_thread_num();
				TopologicalRegularGrid3D::DCellsIterator edges(m_topo_grid, 2, topo_index_partition[thread_num], topo_index_partition[thread_num + 1]);
				for (edges.begin(); edges.valid(); edges.advance()) {
					INDEX_TYPE quad = edges.value();
					if (m_restriction_labels->GetLabel(quad) != 1) continue;
					TopologicalRegularGrid3D::FacetsIterator eit(m_topo_grid);
					bool isgood = true;
...
#pragma omp parallel
			{
				int thread_num = omp_get_thread_num();
				TopologicalRegularGrid3D::DCellsIterator edges(m_topo_grid, 3, topo_index_partition[thread_num], topo_index_partition[thread_num + 1]);
				for (edges.begin(); edges.valid(); edges.advance()) {
					INDEX_TYPE quad = edges.value();
					if (m_restriction_labels->GetLabel(quad) != 1) continue;
					TopologicalRegularGrid3D::FacetsIterator eit(m_topo_grid);
...
#pragma omp parallel for schedule(static) 
		...
		//#pragma omp parallel 
		//	{
		//				AdaptiveEulerAdvector3D t_advector(m_grid, m_func, m_gradient_threshold, m_error_threshold, inside_voxel_critical_advection_checker);
