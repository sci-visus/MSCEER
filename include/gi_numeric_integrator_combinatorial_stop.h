#ifndef NUMERIC_INTEGRATOR_COMBINATORIAL_PATH_COMPRESSING_H
#define NUMERIC_INTEGRATOR_COMBINATORIAL_PATH_COMPRESSING_H
...
namespace GInt {
    ...
    class NumericIntegratorCombinatorialPathCompressing {
    ...
    //        if (!has_extremal) {
    //            printf("ERROR should never get here3\n");
				//Vec3l coords = m_grid->XYZ3d(tid); coords.PrintInt();
    ...
    public:
		...
		NumericIntegratorCombinatorialPathCompressing(RegularGridTrilinearFunction* func, RegularGrid3D* grid, FLOATTYPE error_threshold, FLOATTYPE gradient_threshold, int interation_limit) :
		...
		~NumericIntegratorCombinatorialPathCompressing() {
            delete m_desttype;
            delete m_dest_label;
            delete mCompare;
        }
...
#else 
				for (INDEX_TYPE i = partition[thread_num]; i < partition[thread_num + 1]; i++) {
