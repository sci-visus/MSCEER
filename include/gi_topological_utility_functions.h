...
namespace GInt {
	...
	void RunMeshConsistencyChecks(TopologicalRegularGrid3D* mesh) {

		TopologicalRegularGrid3D::AllCellsIterator allcells(mesh);
		...
		for (allcells.begin(); allcells.valid(); allcells.advance()) {
			...
			{
				TopologicalRegularGrid3D::FacetsIterator fit(mesh);
				for (fit.begin(id); fit.valid(); fit.advance()) {
					INDEX_TYPE facetid = fit.value();

					bool has_reflex = false;
					TopologicalRegularGrid3D::CofacetsIterator cfit(mesh);
			...
			{
				TopologicalRegularGrid3D::CofacetsIterator fit(mesh);
				for (fit.begin(id); fit.valid(); fit.advance()) {
					INDEX_TYPE facetid = fit.value();

					bool has_reflex = false;
					TopologicalRegularGrid3D::FacetsIterator cfit(mesh);
					for (cfit.begin(facetid); cfit.valid(); cfit.advance()) {
						INDEX_TYPE cofacetid = cfit.value();
						if (cofacetid == id) has_reflex = true;
					}
            ...
            // check adjacency
            {
                ...
                TopologicalRegularGrid3D::AdjacentCellsIterator ajit(mesh);
                for (ajit.begin(coords); ajit.valid(); ajit.advance()) {
                    ...
                    bool has_reflex = false;
                    TopologicalRegularGrid3D::AdjacentCellsIterator ajit2(mesh);
            ...
            // check cell verts
            {
                ...
                int countverts = 0;
                TopologicalRegularGrid3D::CellVerticesIterator cvit(mesh);
                ...
                for (cvit.begin(coords); cvit.valid(); cvit.advance()) {
                    ...
                    // check compression and decompression
                    BYTE_TYPE dir = mesh->CompressVertexOffsetToByte(id, cvid);
