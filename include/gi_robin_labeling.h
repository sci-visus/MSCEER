...
namespace GInt {
    ...
    public:
        LowerStar(INDEX_TYPE vertex_cellId, const MeshFunction *topofunc,
            ...
            //TopologicalRegularGrid3D::CellVerticesIterator cviter(m_tgrid);
		...
		int num_unassigned(DiscreteGradientLabeling<Mesh> const*const labeling) const {
		...
		void print_pairing(DiscreteGradientLabeling<Mesh> const*const labeling) const {
    ...
    class RobinsLabelingAlgorithm {
        ...
        DiscreteGradientLabeling<Mesh> *const m_labeling;
        Mesh *m_tgrid;
    ...
    public:
        RobinsLabelingAlgorithm(MeshFunction *const topofunc, Mesh *const tgrid,
			DiscreteGradientLabeling<Mesh> *const labeling) :

            m_topofunc(topofunc), m_tgrid(tgrid), m_labeling(labeling), maxV_labeling(NULL)
        {
        ...
        DIM_TYPE idxOf_maxVertex(INDEX_TYPE cellid) const {
            ...
            DIM_TYPE test_dir = m_tgrid->CompressVertexOffsetToByte(cellid, maxV);

            for (cviter.advance(); cviter.valid(); cviter.advance()) {

                if (Cell_comparator<MeshFunction>::is_greater(cviter.value(), maxV)) {
                    maxV = cviter.value();
                    test_dir = m_tgrid->CompressVertexOffsetToByte(cellid, maxV);
                }
            }
            return test_dir;
        }
        ...
        void create_maxVertex_labeling() {
			if (this->maxV_labeling != NULL) return;
...
#ifdef DEBUG_CRITS
				crit_list.push_back(vert_cellId);
#endif				
#ifdef DEBUG_MODE
                if (debug) {
...
#ifdef DEBUG_CRITS
							crit_list.push_back(alpha->cellid());
#endif
#ifdef DEBUG_MODE
                            if (debug)
...
#ifdef DEBUG_CRITS
					crit_list.push_back(gamma->cellid());
#endif
                    // iterate from the 2nd element
                    for (int k = 1; k < lstar.size(); k++) {
...
#pragma omp critical 
				{
					printf("Lstar of %llu (%f):\n", vert_cellId, this->m_topofunc->cellValue(vert_cellId));
					lstar.print(true);
					lstar.print_pairing(this->m_labeling);
					for (auto id : crit_list){
						printf(" - crit %llu d=%d r=%d v=%f\n", id, this->m_tgrid->dimension(id), this->m_tgrid->restrictionLabel(id), this->m_topofunc->cellValue(id));
...
//
//		void compute_output(MaximumVertexLabeling<Mesh, >) {
//
