...
namespace GInt {
	...
	public:
		...
		DTYPE Dot(const GenericVec2& a) const {
			return (m_v[0] * a[0]) + (m_v[1] * a[1]);
		}
		void Normalize() {
			double mag = 1.0 / this->Mag();
			this->operator*=(mag);
		}
		GenericVec2 operator-(const GenericVec2  &a) const {
			GenericVec2 res(*this);
		...
		GenericVec2 operator*(const int val) const {
			GenericVec2 res(this->m_v[0], this->m_v[1]);// = (*this);
			res.m_v[0] *= val; res.m_v[1] *= val; 
			return res;
		}

		GenericVec2 operator*(const float  val) const {
			GenericVec2<float> res(this->m_v[0], this->m_v[1]);// = (*this);
			res.m_v[0] *= val; res.m_v[1] *= val; 
			return res;
		}
		GenericVec2 operator*(const double  val) const {
			GenericVec2<double> res(this->m_v[0], this->m_v[1]);// = (*this);
			res.m_v[0] *= val; res.m_v[1] *= val; 
			return res;
		}
		...
		operator GenericVec2<float>() const { return GenericVec2<float>(this->m_v[0], this->m_v[1]); }
	...
	typedef GenericVec2<float>			Vec2f;
	...
	public:
		...
		GenericVec3 operator>>(int val) const {
			GenericVec3 other = *this;
			other.m_v[0] >> val;
			other.m_v[1] >> val;
			other.m_v[2] >> val;
			return other;
		...
		void Abs() {
			for (int i = 0; i < 3; i++) this->m_v[i] = abs(this->m_v[i]);
		}
		...
		operator GenericVec3<float>() const { return GenericVec3<float>(this->m_v[0], this->m_v[1], this->m_v[2]); }
		operator GenericVec3<unsigned char>() const { return GenericVec3<unsigned char>(this->m_v[0], this->m_v[1], this->m_v[2]); }
		...
		static GenericVec3<double> Cross(GenericVec3<double>& a, GenericVec3<double>& b) {
			return GenericVec3<double>(a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]);
		}
		...
		GenericVec3<float> FloatFloor() const {
			GenericVec3<float> ddres(floor(this->m_v[0]), floor(this->m_v[1]), floor(this->m_v[2]));
			return ddres;
		}
		// linear interpolation betwee 2 vectors: returns the vector a * (1-t) + b * t
		static GenericVec3<float> Lerp(const GenericVec3<float>& a, const GenericVec3<float>& b, const float t) {
			return (a * (1.0 - t)) + (b * (t));
		}

		static GenericVec3<float> CrossF(GenericVec3<float>& a, GenericVec3<float>& b) {
			return GenericVec3<float>(a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]);
		}

		// returns 2 normalized vectors orthogonal
		static void OrthogonalPlane(GenericVec3<float> v, GenericVec3<float>& b1, GenericVec3<float>& b2) {

			float vmag = v.Mag();
			v = v * (1.0 / vmag); // normalize v
			GenericVec3<float> up(0, 0, 1);

			GenericVec3<float> vXup = CrossF(v, up);
			float vXupmag = vXup.Mag();
			if (vXupmag == 0) {
				b1 = GenericVec3<float>(1, 0, 0);
				b2 = GenericVec3<float>(0, 1, 0);
				return;
			}
