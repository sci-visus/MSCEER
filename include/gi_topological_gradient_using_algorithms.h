...
protected:
        void rec_man_trace_up_marking(INDEX_TYPE& cellid, DIM_TYPE value) {
            struct work {INDEX_TYPE cellid; DIM_TYPE value;};
            std::queue<struct work> todo;
            todo.push({cellid, value});
            while (!todo.empty()) {
                struct work item = todo.front();
                todo.pop();

                if (my_grad_field->getDimAscMan(item.cellid) == item.value) continue;
                my_grad_field->setDimAscMan(item.cellid, item.value);

                INDEX_TYPE current = item.cellid;
                // DIM_TYPE cdim = this->my_mesh_handler->dimension(cellid);
                typename GridType::CofacetsIterator cofacets(my_mesh_handler);
                for (cofacets.begin(current); cofacets.valid(); cofacets.advance()) {
                    INDEX_TYPE temp_id = cofacets.value();
                    ...
                    if (temp_pair == item.cellid) continue;

                    //if (my_mesh_handler->dimension(temp_pair) != cdim) continue;
                    if (my_grad_field->getDimAscMan(temp_id) == item.value) continue;
                    my_grad_field->setDimAscMan(temp_id, item.value);

                    todo.push({temp_pair, item.value});
                }
            }
        }
