...
namespace GInt {
	...
	protected:
		...
		struct ArcCandidate {
			INT_TYPE lowerNodeID;
			INT_TYPE upperNodeID;
		};
		...
		inline INT_TYPE nodeIdForCell(INDEX_TYPE cellID) const {
			typename std::unordered_map<INDEX_TYPE, INT_TYPE>::const_iterator it = nodemap.find(cellID);
			if (it == nodemap.end()) {
				return NULLID;
			}
			return it->second;
	...
	protected:
		...
		INT_TYPE createArc(INDEX_TYPE lowerCellID, INDEX_TYPE upperCellID, std::vector<INDEX_TYPE>& geometry) {
			INT_TYPE lowerNodeID = nodeIdForCell(lowerCellID);
			INT_TYPE upperNodeID = nodeIdForCell(upperCellID);
			if (lowerNodeID == NULLID || upperNodeID == NULLID) {
				printf("ERROR: createArc missing endpoint nodes (%llu, %llu)\n",
					(unsigned long long)lowerCellID, (unsigned long long)upperCellID);
				return NULLID;
			}
		...
		INT_TYPE createArcFromNodeIDs(INT_TYPE lowerNodeID, INT_TYPE upperNodeID) {
			if (lowerNodeID == NULLID || upperNodeID == NULLID) {
				printf("ERROR: createArcFromNodeIDs got null endpoint node\n");
				return NULLID;
			}
			arc<SCALAR_TYPE> at;
			INT_TYPE listID = arcs.size();
...
#pragma omp critical
		...
		void rec_tdcr_candidates(const INDEX_TYPE& cellid, DIM_TYPE& temp_dim, INT_TYPE startNodeID,
			vector<ArcCandidate>& candidates) {
			INDEX_TYPE current = cellid;
			typename MESH_TYPE::FacetsIterator facets(mMesh);
			for (facets.begin(current); facets.valid(); facets.advance()) {
				INDEX_TYPE temp_id = facets.value();
				if (mGrad->getCritical(temp_id)) {
					INT_TYPE lowerNodeID = nodeIdForCell(temp_id);
					...
					INDEX_TYPE pair = mGrad->getPair(temp_id);
					if (pair != cellid && mMesh->dimension(pair) == mMesh->dimension(cellid)) {
						rec_tdcr_candidates(pair, temp_dim, startNodeID, candidates);
					}
				}
			}
		}
		...
		void trace_down_cells_restricted_nogeom_candidates(INDEX_TYPE cellid, INT_TYPE startNodeID,
			vector<ArcCandidate>& candidates) {
			DIM_TYPE temp_dim = mGrad->getDimAscMan(cellid) + 1;
			rec_tdcr_candidates(cellid, temp_dim, startNodeID, candidates);
		}
		void appendCandidatesAsArcs(const vector<ArcCandidate>& candidates) {
			for (size_t i = 0; i < candidates.size(); i++) {
				const ArcCandidate& c = candidates[i];
				createArcFromNodeIDs(c.lowerNodeID, c.upperNodeID);
			}
		}
		...
		void AddArcsThreadLocalNoGeom(INT_TYPE nodeId, vector<ArcCandidate>& candidates) {
			node<SCALAR_TYPE>& n = getNode(nodeId);
			if (n.dim == 0) return;
			if (!mBuildArcAtAll[n.dim - 1]) return;
			if (mBuildArcGeometry[n.dim - 1]) {
				trace_down_cells_restricted(nodes[nodeId].cellindex);
				return;
			}
			trace_down_cells_restricted_nogeom_candidates(nodes[nodeId].cellindex, nodeId, candidates);
		}
		INDEX_TYPE resolve2DNoGeomDestinationCell(INDEX_TYPE startCell,
			DIM_TYPE temp_dim,
			vector<INDEX_TYPE>& traceDest,
			vector<INDEX_TYPE>& pathScratch) {

			pathScratch.clear();
			INDEX_TYPE current = startCell;
			INDEX_TYPE prev = NULLID;

			while (true) {
				if (traceDest[current] != NULLID) {
					INDEX_TYPE cachedDest = traceDest[current];
					for (size_t i = 0; i < pathScratch.size(); i++) {
						traceDest[pathScratch[i]] = cachedDest;
					}
					return cachedDest;
				}
				pathScratch.push_back(current);

				INDEX_TYPE foundCriticalDest = NULLID;
				INDEX_TYPE foundNext = NULLID;
				...
				for (facets.begin(current); facets.valid(); facets.advance()) {
					INDEX_TYPE temp_id = facets.value();
					if (mGrad->getCritical(temp_id)) {
						foundCriticalDest = temp_id;
						transitionCount++;
						continue;
					}
					if (mGrad->getDimAscMan(temp_id) != temp_dim) continue;
					INDEX_TYPE pair = mGrad->getPair(temp_id);
					if (pair == current || mMesh->dimension(pair) != mMesh->dimension(current)) continue;
				...
				if (foundCriticalDest != NULLID) {
					for (size_t i = 0; i < pathScratch.size(); i++) {
						traceDest[pathScratch[i]] = foundCriticalDest;
					}
					return foundCriticalDest;
				}
				prev = current;
				current = foundNext;
			}
	...
	public:
		void ComputeFromGrad(bool restricted = false) {
			...
			fflush(stdout);
			int count = 0;
			const std::chrono::steady_clock::time_point nodesStart = std::chrono::steady_clock::now();
			...
			const std::chrono::steady_clock::time_point nodesEnd = std::chrono::steady_clock::now();
			printAggregateTiming("MSC create nodes", nodesStart, nodesEnd);
			printf(" done! found %d critical points\n", count);
			printf("   -- Finding arcs...");
            fflush(stdout);
			const std::chrono::steady_clock::time_point arcsStart = std::chrono::steady_clock::now();
			INT_TYPE numnodes = nodes.size();
			vector<vector<ArcCandidate> > threadCandidates;
#pragma omp parallel
			{
				int tid = 0;
#pragma omp single
				{
					threadCandidates.resize(omp_get_num_threads());
				}
				tid = omp_get_thread_num();
				vector<ArcCandidate>& localCandidates = threadCandidates[tid];
#pragma omp for
				for (INT_TYPE j = 0; j < numnodes; j++) {
					AddArcsThreadLocalNoGeom(j, localCandidates);
				}
			}
			for (size_t t = 0; t < threadCandidates.size(); t++) {
				appendCandidatesAsArcs(threadCandidates[t]);
			}
			const std::chrono::steady_clock::time_point arcsEnd = std::chrono::steady_clock::now();
			printAggregateTiming("MSC create arcs", arcsStart, arcsEnd);
            printf(" done! found %d arcs\n", this->arcs.size());
		}
		void BuildArcs2DNoGeomDenseCache() {
			// NOTE: Experimental 2D dense destination cache path.
			// In release tests (1024x1024 random and masked-random fields, 4 threads),
			// this path measured roughly 2x slower for arc creation than ComputeFromGrad().
			typename MESH_TYPE::AllCellsIterator t_cells(mMesh);
			printf("   -- Finding critical points...");
			fflush(stdout);
			int count = 0;
			...
			fflush(stdout);
			const std::chrono::steady_clock::time_point arcsStart = std::chrono::steady_clock::now();
			const INDEX_TYPE numCells = mMesh->numCells();
			vector<INT_TYPE> cellToNodeId(numCells, NULLID);
			for (INT_TYPE nid = 0; nid < (INT_TYPE)nodes.size(); nid++) {
				cellToNodeId[nodes[nid].cellindex] = nid;
			}
			vector<INDEX_TYPE> traceDest(numCells, NULLID);
			vector<ArcCandidate> candidates;
			INT_TYPE numnodes = nodes.size();
			vector<INDEX_TYPE> pathScratch;

			for (INT_TYPE nodeId = 0; nodeId < numnodes; nodeId++) {
				node<SCALAR_TYPE>& n = nodes[nodeId];
				if (n.dim == 0) continue;
				if (!mBuildArcAtAll[n.dim - 1]) continue;
				if (mBuildArcGeometry[n.dim - 1]) {
					trace_down_cells_restricted(n.cellindex);
					continue;
				}

				const DIM_TYPE temp_dim = mGrad->getDimAscMan(n.cellindex) + 1;
				INDEX_TYPE destCell = resolve2DNoGeomDestinationCell(n.cellindex, temp_dim, traceDest, pathScratch);
				if (destCell == NULLID) {
					trace_down_cells_restricted_nogeom_candidates(n.cellindex, nodeId, candidates);
					continue;
				}

				INT_TYPE lowerNodeID = cellToNodeId[destCell];
				if (lowerNodeID == NULLID) continue;
				ArcCandidate c;
				c.lowerNodeID = lowerNodeID;
				c.upperNodeID = nodeId;
				candidates.push_back(c);
			}
			appendCandidatesAsArcs(candidates);
			const std::chrono::steady_clock::time_point arcsEnd = std::chrono::steady_clock::now();
			printAggregateTiming("MSC create arcs", arcsStart, arcsEnd);
			printf(" done! found %d arcs\n", this->arcs.size());
		}
    ...
    void SetPersistanceByNumOfMaxima(int num_maxima){
      ...
      for (int i = 0; i < this->mCRecords.size(); i++) {
		...
		virtual void ComputeHierarchy(SCALAR_TYPE pers_limit) {
			...
			const std::chrono::steady_clock::time_point hierarchyEnd = std::chrono::steady_clock::now();
			printAggregateTiming("MSC hierarchy", hierarchyStart, hierarchyEnd);
