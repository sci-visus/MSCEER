...
namespace GInt {
	...
	class TopologicalConverventGradientBuilder : public TopologicalRegionGrowingSimpleGradientBuilder<MeshType, FuncType, GradType>  {
	public:
		//struct MemberDist;
	protected:
		...
		virtual bool mdDecrement(const INDEX_TYPE& cellid, const DIM_TYPE& dim) {
			...
			typename MeshType::FacetsIterator facets(this->mMesh);
		...
		virtual void initAssigned() {
			typename MeshType::AllCellsIterator allit(this->mMesh);
		...
		virtual void seedQueueWithDMinima(const DIM_TYPE& dim) {
			int counter = 0;
			typename MeshType::DCellsIterator d_cells(this->mMesh, dim);
		...
		virtual int countMDCofacets(const INDEX_TYPE& cellid) {
			int count = 0;
			typename MeshType::CofacetsIterator cofacets(this->mMesh);
		...
		virtual int pickFromCandidates(const INDEX_TYPE& cellid,
			...
			typename FuncType::DType cell_value = this->mFunc->cellValue(cellid);
		...
		virtual void pickAndPair(const typename TopologicalRegionGrowingSimpleGradientBuilder< MeshType,  FuncType,  GradType>::comparison_element& element,
			const DIM_TYPE& dim) {
	...
	public:
		...
		TopologicalConverventGradientBuilder(
			...
			GradType* grad_field) :
      TopologicalRegionGrowingSimpleGradientBuilder<MeshType, FuncType, GradType>(mesh_function, mesh_handler, grad_field) {
			my_erase = true;
