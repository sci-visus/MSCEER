...
#ifndef NUMERIC_INTEGRATOR_EXPANDING_REGION_STOP_FILTERED_H
#define NUMERIC_INTEGRATOR_EXPANDING_REGION_STOP_FILTERED_H
...
namespace GInt {
    ...
    class NumericIntegratorExpandingRegionStopFiltered {
    ...
    public:
        ...
        NumericIntegratorExpandingRegionStopFiltered(RegularGridTrilinearFunction* func, RegularGrid3D* grid,  FLOATTYPE error_threshold, FLOATTYPE gradient_threshold, int interation_limit) :
            ...
            m_filterValue = std::numeric_limits<FLOATTYPE>::max();
        }

        ~NumericIntegratorExpandingRegionStopFiltered() {
        ...
        void BeginIntegration(bool verbose = true) {
...
#pragma omp parallel for
            for (INDEX_TYPE i = 0; i < t_num_vertices; i++) {
                ...
                if ( fabs(m_func->SampleImage(i)) <= m_filterValue ) {
                    m_destinations->SetLabel(i, -2);
                    m_certains->SetLabel(i, -2);
                    continue;
                }
...
#pragma omp for schedule(dynamic)  nowait
...
#ifdef OUTPUTINTERMEDIATE
            {
                ...
                m_destinations->OutputToIntFile("dest_original.raw");
            }
...
#pragma omp single
            {
                ...
                m_destinations->OutputToIntFile("dests_after_first_integration.raw");
#endif
                if(verbose){
                    printf(" done!\n");
...
#pragma omp for schedule(static)
            for (INDEX_TYPE i = 0; i < t_num_vertices; i++) {
                // early skip if this is already a maximum or vacuum region
...
#ifdef OUTPUTINTERMEDIATE
                m_certains->OutputToFile("certain_expansion2.raw");
                m_certains->OutputToFile("certain_after_boundary_test.raw");
#endif
...
#pragma omp for schedule(guided)  nowait
            ...
            m_destinations->OutputToIntFile("dests_after_reintegration.raw");
#endif
            if(verbose){
                printf(" done!\n");
            }
