...
#pragma omp single
	...
	protected:
		...
		int InspectPriorRegions(INDEX_TYPE currentVID) {
			...
			Vec3l neighbors[6];
			int num_neighbors = m_grid->GatherExistingNeighborsSameBdry6(m_grid->Coords(currentVID), neighbors);
			for (int i = 0; i < num_neighbors; i++) {
				INDEX_TYPE neighborVID = m_grid->Index3d(neighbors[i]);
		...
		void Expand_Lower_Neighborhood(INDEX_TYPE startid, int start_label) {
			...
			while (!growing_front.empty()) {
				...
				if (cellvale >= 0) {
					...
					Enqueue_Later_Neighbors(m_grid->Coords(currid), growing_front, enqueued_set);
				}
...
//#define DEBUGGSERB
	...
	protected:
...
#endif
		{
			...
			Vec3l neighbors[6];
			int num_neighbors = m_grid->GatherExistingNeighborsAll6(m_grid->Coords(currentVID), neighbors);
			for (int i = 0; i < num_neighbors; i++) {
				INDEX_TYPE neighborVID = m_grid->Index3d(neighbors[i]);
				if (AComesBeforeB(neighborVID, currentVID)) {
					if (m_dest_label->GetLabel(neighborVID) == -1) {
