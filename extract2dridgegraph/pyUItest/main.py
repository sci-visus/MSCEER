...
class CanvasImage:
    ...
    def __init__(self, placeholder, path):
        ...
        #self.canvas.bind('<Motion>', self.__motion) # to queries
...
class GeometricGraph:
    ...
    def AddNode(self, nid, x, y):
        node = GraphNode(nid, x, y)
        self._nid_to_node[nid] = node
    ...
    def GetNode(self, nid):
        return self._nid_to_node[nid]

    def AddArc(self, nid1, nid2, points):
        node1 = self.GetNode(nid1)
        node2 = self.GetNode(nid2)
        arc = GraphArc(node1, node2, points)
        node1._arcs.append(arc)
        ndoe2._arcs.append(arc)
        return arc

    def GetArcs(self, nid):
        return self._nid_to_node[nid]._arcs

    def LoadFromBaseName(self, filenamebase):
        nodes_name = filename + ".nodes.txt"
        arcs_name = filename + ".arcs.txt"
        nodesfile = open(nodes_name, 'r')
        nodeslines = nodesfile.readlines()
        nodesfile.close()

        for line in nodeslines:
            tokens = line.split(',')
            self.AddNode(*[int(x) for x in tokens])

        arcsfile = open(arcs_name, 'r')
        arcslines = arcsfile.readlines()
        arcsfile.close()

        for line in arcslines:
            tokens = line.split(',')
            points = [float(x) for x in tokens[3:]]
            points = np.reshape(points, (2, len(points)/2)).tolist()
            self.AddArc(int(tokens[1]), int(tokens[2]), points)
...
class ImageOverlayScene(Zoom):
    ...
    def __init__(self, placeholder, path):
        Zoom.__init__(self, placeholder, path)
...
class MainWindow(ttk.Frame):
    ...
    def __init__(self, mainframe, path):
        ...
        self.master.columnconfigure(0, weight=1)
        canvas = Zoom(self.master, path)  # create widget
        canvas.grid(row=0, column=0)  # show widget


filename = 'C:/Users/jediati/Desktop/1_E9hxKdAMNUucJOhQeeA8yA.jpeg' # place path to your image here
print(filename)
...
path = filename
root = tk.Tk()
app = ImageOverlayScene(root, path=path)
root.mainloop()