...
int main(int argc, char** argv) {
	...
	if (argc < 4) { printf("Usage: filename X Y \n"); return 0; }
	...
	GridFuncType* grid_func = new GridFuncType(grid, buff);
	printf("making mesh...\n");
	...
	printf("outputting mesh function...\n");

	auto num_cells = mesh->numCells();
	float* fbuff = new float[num_cells];

#pragma omp for
	for (INDEX_TYPE i = 0; i < num_cells; i++) {
		fbuff[i] = mesh_func->cellValue(i);
	}

	char outname[2048];
	sprintf(outname, "%s.dat", argv[1]);
	FILE* outf = fopen(outname, "wb");
	fwrite(fbuff, sizeof(float), num_cells, outf);
	fclose(outf);
	return 1;
}
